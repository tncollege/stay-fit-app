...
export default function WorkoutView({
  ...
  const [workoutPlanOverride, setWorkoutPlanOverride] = useState<'today' | 'yesterday' | null>(null);
...
const allWorkouts = useMemo<Workout[]>(() => {
  ...
  const activeWorkoutPlan = workoutPlanOverride === 'yesterday' && yesterdayPlan ? yesterdayPlan : todayPlan;
  ...
  const recommendedMode = useMemo(
    () => getRecommendedTrainingMode(workoutReadiness, fatigueStatus, todayName, activeWorkoutPlan),
  ...
  }, [activeTrainingMode, workoutReadiness, fatigueStatus, activeWorkoutPlan?.planName, workoutEnginePlanName]);
...
function WeeklyPlanSection(props: any) {
  ...
  const yesterdayPlanExercises = yesterdayPlan?.exercises || [];
  const yesterdayPlanName = String(yesterdayPlan?.planName || '').trim();
  const originalTodayPlanName = String(originalTodayPlan?.planName || todayPlan?.planName || '').trim();
  const hasYesterdayPlan = Boolean(yesterdayPlanName || yesterdayPlanExercises.length);
  const yesterdayPlanCompleted = hasYesterdayPlan && (yesterdayWorkouts || []).some((workout: any) => {
    const workoutName = normalizeForMatch(getWorkoutDisplayName(workout));
    const planName = normalizeForMatch(yesterdayPlanName);
    const workoutExercises = (workout?.sets || []).map((set: any) => normalizeForMatch(set.exercise || ''));
    const planExercises = yesterdayPlanExercises.map((ex: any) => normalizeForMatch(ex.name || ''));

    if (planName && workoutName.includes(planName)) return true;
    if (!planExercises.length) return false;
    return planExercises.some((exerciseName: string) => workoutExercises.includes(exerciseName));
  });
  const missedYesterdayPlan = hasYesterdayPlan && !yesterdayPlanCompleted && workoutPlanOverride !== 'yesterday';
      ...
      {(missedYesterdayPlan || isUsingYesterdayPlan) && (
        <div className={`rounded-2xl border p-4 space-y-4 ${
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
              <div className="text-[10px] font-black uppercase tracking-widest text-orange-300">Missed Workout Recovery</div>
              <div className="text-sm font-black mt-1">
                {isUsingYesterdayPlan
                  ? `Training yesterday’s ${yesterdayPlanName || 'workout'} today`
                  : `Looks like yesterday’s ${yesterdayPlanName || 'workout'} was missed`}
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            >
              Do Yesterday’s {yesterdayPlanName || 'Workout'}
            </button>
            ...
            <button
              onClick={() => {
                setWorkoutPlanOverride?.(null);
                setEditingPlanDay(yesterdayName || todayName);
