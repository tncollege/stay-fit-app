...
const FORM_TIPS: Record<string, any> = {
  "lat pulldown": {
    ...
    mistakes: ["Leaning too far back", "Using momentum", "Partial reps"],
  ...
  "barbell curl": {
    cues: ["Keep elbows fixed", "Full stretch at bottom", "Controlled tempo"],
    mistakes: ["Swinging body", "Using shoulders", "Half reps"],
...
const WORKOUT_WINDOW_MAP: Record<
  string,
  { label: string; estimatedStart: string }
> = {
...
function getPlanTrainingMode(plan: any): TrainingMode | null {
  ...
  if (/leg|lower|quad|hamstring|glute|calf|squat|lunge|core/.test(planName))
    return "Legs";
  if (/push|chest|tricep|shoulder|bench|press/.test(planName)) return "Push";
  if (/pull|back|bicep|row|pulldown|pullup|chinup|deadlift/.test(planName))
    return "Pull";
  ...
  if (
    /leg|lower|quad|hamstring|glute|calf|squat|lunge|leg press|leg curl|leg extension|free squat|core|crunch|leg raise/.test(
      planText,
    )
  )
    return "Legs";
  if (/push|chest|tricep|shoulder|bench|press|dip|fly|raise/.test(planText))
    return "Push";
  if (
    /pull|back|bicep|row|pulldown|pullup|chinup|deadlift|barbell curl|dumbbell curl|hammer curl/.test(
      planText,
    )
  )
    return "Pull";
...
function getModePrescription(
  ...
  if (mode === "Recovery") {
    return {
      ...
      instruction:
        "Avoid heavy loading. Use mobility, walking, stretching and light pump work.",
    };
  }
  ...
  if (score >= 75 && fatigueStatus === "Normal Fatigue") {
    return {
      ...
      instruction:
        "Push progressive overload. Add load or reps where form is stable.",
    };
  }
  ...
  if (score >= 55) {
    return {
      ...
      instruction:
        "Train with controlled volume. Keep 1–3 reps in reserve and avoid max attempts.",
    };
  }
...
function getWorkoutDisplayName(w: any) {
  ...
  const exercises = (w.sets || []).map((s: any) =>
    String(s.exercise || "").toLowerCase(),
  );
  ...
  const hasBack =
    ...
    exercises.some(
      (e: string) =>
        e.includes("row") || e.includes("pulldown") || e.includes("pull"),
    );
  const hasBiceps =
  ...
  const hasChest =
    ...
    exercises.some(
      (e: string) =>
        e.includes("bench") || e.includes("chest") || e.includes("fly"),
    );
  const hasShoulders =
    muscles.includes("Shoulders") ||
    exercises.some(
      (e: string) => e.includes("shoulder") || e.includes("raise"),
    );
  const hasTriceps = exercises.some(
    (e: string) =>
      e.includes("tricep") || e.includes("pushdown") || e.includes("extension"),
  );
  const hasLegs =
    muscles.includes("Legs") ||
    exercises.some(
      (e: string) =>
        e.includes("squat") || e.includes("leg") || e.includes("lunge"),
    );
...
function getDynamicCardioMetric(value: string) {
  ...
  if (ex.includes("cycling") || ex.includes("bike") || ex.includes("rowing"))
    return "Resistance";
...
function getCardioCaloriesPerMinute(exercise: string, fatigueStatus?: string) {
  ...
  if (ex.includes("run") || ex.includes("hiit") || ex.includes("jump"))
    base = 10;
  if (ex.includes("cycling") || ex.includes("bike") || ex.includes("row"))
    base = 8;
...
function getConditioningAdvice(
  readiness: number,
  fatigueStatus: string,
  mode: "Cardio" | "Sports" | "Yoga",
) {
  if (mode === "Yoga") {
    if (readiness < 55 || fatigueStatus === "High Fatigue")
      return "Recovery flow: keep it easy, nasal breathing, long holds.";
    if (readiness > 75 && fatigueStatus === "Normal Fatigue")
      return "Performance flow: add mobility, balance and controlled strength holds.";
    return "Controlled flow: focus on mobility, breath and joint quality.";
  }

  if (readiness < 55 || fatigueStatus === "High Fatigue")
    return "Low-intensity zone today. Avoid hard intervals and keep the session restorative.";
  if (readiness > 75 && fatigueStatus === "Normal Fatigue")
    return "Good day for higher output. Intervals or tempo work can be added.";
...
function workoutMatchesPlan(workout: any, plan: any) {
  if (!workout || !plan) return false;

  const planName = normalizeForMatch(plan?.planName || "");
  const workoutName = normalizeForMatch(getWorkoutDisplayName(workout));
  const rawWorkoutName = normalizeForMatch(workout?.name || "");

  if (planName && (workoutName.includes(planName) || rawWorkoutName.includes(planName))) {
    return true;
  }

  const workoutExercises = (workout?.sets || []).map((set: any) =>
    normalizeForMatch(set.exercise || ""),
  );
  const planExercises = (plan?.exercises || []).map((ex: any) =>
    normalizeForMatch(ex.name || ""),
  );

  if (!planExercises.length || !workoutExercises.length) return false;

  const matches = planExercises.filter((exerciseName: string) =>
    workoutExercises.includes(exerciseName),
  ).length;

  return matches >= Math.min(2, planExercises.length);
}
...
export default function WorkoutView({
  ...
  const [completedPlanExercises, setCompletedPlanExercises] = useState<
    Record<string, boolean>
  >({});
  const [trainingMode, setTrainingMode] = useState<TrainingMode>("Auto");
  const [workoutPlanOverride, setWorkoutPlanOverride] = useState<
  ...
  const workoutStartTime =
    todaysWorkoutSchedule.startTime ||
    todaysWorkoutSchedule.estimatedStart ||
    performanceEngine?.workout?.startTime ||
    null;
  const selectedWorkoutWindow =
    todaysWorkoutSchedule.window ||
    getWorkoutWindowFromTime(workoutStartTime || "") ||
    performanceEngine?.workout?.window ||
    "";

  const saveWorkoutTimingForDate = (payload: {
    window?: string;
    startTime?: string;
    ...
    try {
      const nextPayload = {
        ...
        window:
          ...
          getWorkoutWindowFromTime(payload.startTime || workoutStartTime || ""),
        startTime:
          payload.startTime ||
          workoutStartTime ||
          payload.estimatedStart ||
          null,
        estimatedStart:
          payload.estimatedStart ||
          payload.startTime ||
      ...
      localStorage.setItem(
        "stayfit_workout_time_context",
        JSON.stringify(nextPayload),
      );
      localStorage.setItem(
        "stayfit_workout_window_context",
        JSON.stringify(nextPayload),
      );
    } catch {}
  };
  ...
  const setWorkoutWindowForDate = (windowKey: string) => {
    ...
    showWorkoutMessage(
      `Workout time set: ${selected.label} • ${formatWorkoutTime(selected.estimatedStart)}`,
    );
  };
  ...
  const yesterdayName = dayNames[new Date(yesterdayKey + "T00:00:00").getDay()];
  ...
  const yesterdayPlan = workoutPlans[yesterdayName];

  const pendingMissedWorkout = useMemo(() => {
    const baseDate = new Date(viewDate + "T00:00:00");

    for (let offset = 1; offset <= 7; offset++) {
      const missedDate = new Date(baseDate);
      missedDate.setDate(missedDate.getDate() - offset);
      const missedDateKey = getLocalDateKey(missedDate);
      const missedDayName = dayNames[missedDate.getDay()];
      const plan = workoutPlans[missedDayName];
      const hasPlan = Boolean(
        String(plan?.planName || "").trim() || plan?.exercises?.length,
      );

      if (!hasPlan) continue;

      let completedLater = false;
      for (let scanOffset = offset; scanOffset >= 1; scanOffset--) {
        const scanDate = new Date(baseDate);
        scanDate.setDate(scanDate.getDate() - scanOffset);
        const scanDateKey = getLocalDateKey(scanDate);
        const dayWorkouts = data.workouts?.[scanDateKey] || [];

        if (dayWorkouts.some((workout: any) => workoutMatchesPlan(workout, plan))) {
          completedLater = true;
          break;
        }
      }
      ...
      if (!completedLater) {
        return {
          dateKey: missedDateKey,
          dayName: missedDayName,
          plan,
          daysAgo: offset,
        };
      }
  ...
  const todayWorkoutBurn = workoutsArr.reduce(
  ...
  const workoutReadiness = useMemo(() => {
    if (performanceEngine?.readiness?.score)
      return Math.round(Number(performanceEngine.readiness.score));
    ...
    const strainScore =
      todayWorkoutBurn > 0 || stepsToday > 0
        ...
        : 55;
    const nutritionScore = mealsToday.length
      ? Math.min(100, (proteinToday / proteinTarget) * 100)
      : 55;
    const hydrationScore =
      waterTotal > 0 ? Math.min(100, (waterTotal / 3.5) * 100) : 60;
  ...
  const recommendedMode = useMemo(
    () =>
      getRecommendedTrainingMode(
        workoutReadiness,
        fatigueStatus,
        todayName,
        activeWorkoutPlan,
      ),
    [workoutReadiness, fatigueStatus, todayName, activeWorkoutPlan],
  );

  const activeTrainingMode =
    trainingMode === "Auto" ? recommendedMode : trainingMode;
  const workoutEnginePlanName = useMemo(() => {
    const planName = String(activeWorkoutPlan?.planName || "").trim();
    return planName || activeTrainingMode;
  }, [activeWorkoutPlan?.planName, activeTrainingMode]);

  const workoutPrescription = useMemo(() => {
    const base = getModePrescription(
      activeTrainingMode,
      workoutReadiness,
      fatigueStatus,
    );
    if (!activeWorkoutPlan?.planName) return base;
    return {
      ...base,
      title:
    ...
    workoutEnginePlanName,
  ]);

  const workoutRecommendationLabel =
    trainingMode === "Auto" ? workoutEnginePlanName : activeTrainingMode;
  ...
  const getExerciseOptions = (category: string, subcategory?: string) => {
    ...
    return Array.from(
      new Set(
        [...fromBase, ...fromPersonal].map((e) => toExerciseTitle(String(e))),
      ),
    );
  };
  ...
  const getPreviousPerformance = (exerciseName: string) => {
    ...
    const matchingSets = allWorkouts
      .flatMap((w: any) =>
        (w.sets || []).map((s: any) => ({ ...s, workoutName: w.name })),
      )
      .filter(
        (s: any) =>
          String(s.exercise || "").toLowerCase() === exerciseName.toLowerCase(),
      );
    ...
    const suggestedWeight =
      Number(last.weight || 0) > 0 ? Number(last.weight || 0) + 2.5 : 0;
    const suggestedReps =
      Number(last.reps || 0) > 0 ? Number(last.reps || 0) + 1 : 0;
  ...
  const addExerciseToPlan = () => {
    ...
    if (
      existing.some((ex: any) => ex.name.toLowerCase() === name.toLowerCase())
    )
      return;
  ...
  const saveCurrentPlan = async () => {
    ...
    try {
      ...
      showWorkoutMessage(
        "Unable to save weekly plan. Please try again.",
        "error",
      );
    } finally {
      setSavingPlan(false);
  ...
  const startTodayPlan = () => {
    ...
    setWorkoutName(activeWorkoutPlan.planName || todayName + " Workout");

    const first = activeWorkoutPlan.exercises?.[0];
    if (first) {
    ...
    setActiveTab("strength");
    showWorkoutMessage(`${activeWorkoutPlan.planName || "Workout"} loaded`);
  };
  ...
  const handleAddSet = () => {
    ...
    // Instant UI update first. Do not wait for AI/cloud before the set appears.
    setSavingSet(true);
    ...
    // Start the timer immediately with a safe default so it never feels delayed.
    const defaultRestSeconds =
      Number(rpe) >= 9 ? 120 : Number(rpe) >= 8 ? 90 : 60;
    setTimerActive(false);
    setTimerTime(defaultRestSeconds);
    setRecoveryReason(
      "Smart rest started instantly. AI may adjust this recommendation.",
    );
    window.setTimeout(() => setTimerActive(true), 0);

    setWeight("");
    setReps("");
    showWorkoutMessage("Set saved successfully");
    window.setTimeout(() => setSavingSet(false), 250);

    // AI recovery timing runs in the background and only refines the active timer.
    const requestStartedAt = Date.now();
    const recoveryWeight =
      finalWeight === "0" ? "bodyweight" : `${finalWeight}kg`;
    calculateRecoveryTime(finalExercise, recoveryWeight, finalReps)
      .then((recovery) => {
        if (!recovery) return;
        ...
        const adjustedSeconds = Math.max(
          15,
          Number(recovery.seconds || defaultRestSeconds) - elapsed,
        );
        setTimerTime((current: number) => Math.max(current, adjustedSeconds));
        setRecoveryReason(recovery.reason || "AI-adjusted rest recommendation");
        setTimerActive(true);
      })
      .catch((err) => console.error("Recovery timer failed", err));
  };
  ...
  const handleAddCardio = async () => {
    ...
    const extraParam =
      cardioExtraMetric && cardioExtraValue
        ? { [cardioExtraMetric]: cardioExtraValue }
        : {};
      ...
      sets: [
        {
          ...
          distance: shouldShowDistance(cardioExercise)
            ? cardioDistance
            : undefined,
          ...extraParam,
        },
  ...
  const handleFinishWorkout = async () => {
    ...
    const workoutMuscles = Array.from(
      new Set(
        currentSets.map((s: any) => s.muscle || selectedMuscle).filter(Boolean),
      ),
    );
    ...
    const finalWorkoutName =
      workoutName && workoutName.trim().length > 1
        ? workoutName.trim()
        : autoWorkoutName;
  ...
  const handleAddSportsOrYoga = async (category: "Sports" | "Yoga") => {
    ...
    const newWorkout: Workout = {
      ...
      muscles: [category],
      sets: [
        { exercise: toExerciseTitle(cardioExercise), duration: durationNum },
      ],
  ...
  const handleDeleteWorkout = async (id: string | number) => {
    setData((prev: AppData) => ({
      ...
      workouts: {
        ...prev.workouts,
        [viewDate]: (prev.workouts[viewDate] || []).filter(
          (w: any) => w.id !== id,
        ),
      },
    }));
  ...
  const handleDeletePersonalExercise = (name: string) => {
    setData((prev: AppData) => ({
      ...prev,
      personalExercises: (prev.personalExercises || []).filter(
        (e: any) => e.name !== name,
      ),
    }));
  };
            ...
            <div className="stat-card space-y-8">
              ...
              {hasLoggedTodayWorkout ? (
                <TodayWorkoutSummary
                  ...
                  onExtraSession={startTrainingSession}
                />
              ) : (
                <WeeklyPlanSection
                  dayNames={dayNames}
                  ...
                  pendingMissedWorkout={pendingMissedWorkout}
                  workoutPlanOverride={workoutPlanOverride}
                  setWorkoutPlanOverride={setWorkoutPlanOverride}
      ...
      {showTipsFor && (
        ...
        >
          ...
          >
            ...
            {(() => {
              const normalized =
                normalizeExerciseName(showTipsFor).toLowerCase();

              const key = Object.keys(FORM_TIPS).find(
              ...
              if (!tips) {
                return (
                  <div className="text-xs text-white/70 leading-relaxed">
                    Keep your form controlled, use full range of motion, and
                    avoid momentum.
                  </div>
                );
              }
              ...
              return (
                <>
                  {/* CUES */}
                  <div className="mb-4">
                    <div className="text-[10px] uppercase text-lime font-bold mb-1">
                      Key Cues
                    </div>
                  ...
                  {/* MISTAKES */}
                  <div className="mb-4">
                    <div className="text-[10px] uppercase text-pink font-bold mb-1">
                      Avoid
                    </div>
    ...
    <section className="rounded-[2rem] border border-lime/20 bg-panel/80 p-5 shadow-xl shadow-lime/5 space-y-5">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="grid grid-cols-3 gap-3 text-center min-w-[280px]">
          <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
            <div className={`text-2xl font-black ${readinessColor}`}>
              {readiness}%
            </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
            <div className="text-sm font-black text-lime">
              {weeklyTrainingLoad}
            </div>
            <div className="text-[8px] uppercase tracking-widest text-white/30 font-black">
              7D Load
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <MiniMetric
          ...
          value={recommendationLabel || recommendedMode}
        />
      </div>
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {TRAINING_MODES.map((mode) => (
            ...
            >
              {mode === "Auto"
                ? `Auto: ${recommendationLabel || recommendedMode}`
                : mode}
            </button>
          ))}
        </div>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <SummaryCard
        ...
        value={trainedMuscles.length ? trainedMuscles.join(", ") : "—"}
      />
      <SummaryCard
        label="Active Session"
        value={currentSets.length ? "In Progress" : "Not Started"}
      />
    </div>
  );
}
...
function TodayWorkoutSummary({
  ...
  const totalSets = (workouts || []).reduce(
    (sum: number, w: any) =>
      sum + ((w.sets || []).length || Number(w.totalSets || 0)),
    0,
  );
  const totalVolume = (workouts || []).reduce(
    (sum: number, w: any) => sum + Number(w.volume || w.totalVolume || 0),
    0,
  );
  const calories = (workouts || []).reduce(
    (sum: number, w: any) => sum + Number(w.caloriesBurned || w.calories || 0),
    0,
  );
  ...
  return (
    <div className="rounded-[2rem] border border-lime/20 bg-lime/[0.04] p-5 space-y-5">
      <div>
        <div className="label-small text-lime mb-2">
          Today’s Workout Summary
        </div>
        <h3 className="text-2xl font-black tracking-tight">{title}</h3>
        <p className="text-[11px] text-white/40 mt-1">
          Today’s workout is already logged. The plan is hidden now; focus on
          recovery, hydration and post-workout nutrition.
        </p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <SummaryCard
          label="Volume"
          value={totalVolume ? `${Math.round(totalVolume)} kg` : "—"}
        />
        <SummaryCard
          label="Calories"
...
function WeeklyPlanSection(props: any) {
  const {
    ...
    yesterdayPlan,
    yesterdayName,
    pendingMissedWorkout,
    workoutPlanOverride,
    setWorkoutPlanOverride,
    planEditorOpen,
    setPlanEditorOpen,
    editingPlanDay,
    setEditingPlanDay,
  ...
  const missedPlan = pendingMissedWorkout?.plan || yesterdayPlan;
  const missedPlanExercises = missedPlan?.exercises || [];
  const missedPlanName = String(missedPlan?.planName || "").trim();
  const missedPlanDayName = pendingMissedWorkout?.dayName || yesterdayName;
  const missedPlanDateKey = pendingMissedWorkout?.dateKey || null;
  const originalTodayPlanName = String(
    originalTodayPlan?.planName || todayPlan?.planName || "",
  ).trim();
  const hasMissedPlan = Boolean(missedPlanName || missedPlanExercises.length);
  const missedYesterdayPlan =
    hasMissedPlan &&
    Boolean(pendingMissedWorkout) &&
    workoutPlanOverride !== "yesterday";
  const isUsingYesterdayPlan = workoutPlanOverride === "yesterday";
  const hasLoggedSetForExercise = (name: string) =>
    (currentSets || []).some(
      (set: any) =>
        normalizeForMatch(set.exercise || "") === normalizeForMatch(name),
    );
  const yesterdayNames = (yesterdayWorkouts || [])
    .map((w: any) => getWorkoutDisplayName(w))
    .filter(Boolean);
  const yesterdaySummary =
    yesterdayNames.length > 1 ? yesterdayNames.join(" + ") : yesterdayNames[0];
  const { setShowTipsFor } = props;
  const yesterdayNote =
    yesterdaySummary && originalTodayPlanName && !isUsingYesterdayPlan
      ? `Yesterday you logged ${yesterdaySummary}. Today is planned as ${originalTodayPlanName}. Update the weekly plan if your split changed.`
      : "";

  return (
    <div
      data-weekly-plan-section
      className="rounded-[2rem] border border-lime/20 bg-lime/[0.03] p-5 space-y-5"
    >
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          <div className="label-small text-lime mb-2">
            Today’s Workout Plan • {todayName}
          </div>
          <h3 className="text-2xl font-black tracking-tight">
            {todayPlan?.planName || "No plan set for today"}
          </h3>
          <p className="text-[11px] text-white/40 mt-1">
            Begin the weekly plan, log sets, then complete exercises when at
            least one set is recorded.
          </p>
        </div>
      ...
      {(missedYesterdayPlan || isUsingYesterdayPlan) && (
        <div
          className={`rounded-2xl border p-4 space-y-4 ${
            isUsingYesterdayPlan
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
              ...
              <div className="text-sm font-black mt-1">
                {isUsingYesterdayPlan
                  ? `Training pending ${missedPlanName || "workout"} today`
                  : `Looks like ${missedPlanDayName || "your previous"}’s ${missedPlanName || "workout"} is pending`}
              </div>
              <div className="text-[11px] text-white/45 mt-1 leading-relaxed">
                {missedPlanDateKey
                  ? `Carry forward the missed ${missedPlanDayName} session from ${missedPlanDateKey}, continue today’s split, or edit the weekly plan.`
                  : "Choose whether to carry forward the missed session, continue today’s split, or edit the weekly plan."}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <button
              onClick={() => {
                setWorkoutPlanOverride?.("yesterday");
                setPlanEditorOpen(false);
              }}
              className={`rounded-xl px-3 py-3 text-[9px] font-black uppercase tracking-widest border transition-all ${
                isUsingYesterdayPlan
                  ? "bg-orange-400 text-dark border-orange-400 shadow-lg shadow-orange-400/20"
                  : "bg-orange-400/15 text-orange-300 border-orange-400/30 hover:bg-orange-400 hover:text-dark"
              }`}
            >
              Do Pending {missedPlanName || "Workout"}
            </button>

            <button
              onClick={() => setWorkoutPlanOverride?.("today")}
              className="rounded-xl px-3 py-3 text-[9px] font-black uppercase tracking-widest border bg-white/[0.03] text-white/60 border-border hover:text-white"
            >
              Continue Today’s {originalTodayPlanName || "Plan"}
            </button>

            <button
              onClick={() => {
                setWorkoutPlanOverride?.(null);
                setEditingPlanDay(missedPlanDayName || todayName);
                setPlanEditorOpen(true);
              }}
              className="rounded-xl px-3 py-3 text-[9px] font-black uppercase tracking-widest border bg-sky/10 text-sky border-sky/20 hover:bg-sky hover:text-dark"
            >
              Edit Plan
            </button>
          </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-black/20 p-4 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-white/40">
              When will you train today?
            </div>
            <div className="text-[11px] text-white/35 mt-1">
              Nutrition will recalculate pre/post-workout meals whenever this
              time changes.
            </div>
          </div>
          {workoutStartTime && (
            <div className="shrink-0 rounded-xl border border-lime/20 bg-lime/10 px-3 py-2 text-right">
              <div className="text-[8px] font-black uppercase tracking-widest text-lime/70">
                Training Time
              </div>
              <div className="text-sm font-black text-lime leading-none mt-1">
                {formatWorkoutTime(workoutStartTime)}
              </div>
            </div>
          )}
        </div>
          ...
          {Object.entries(WORKOUT_WINDOW_MAP).map(([key, item]: any) => (
            ...
            >
              ...
              <span className="block text-[9px] opacity-60 mt-1">
                {formatWorkoutTime(item.estimatedStart)}
              </span>
            </button>
          ))}
        </div>
        ...
        <label className="block rounded-2xl border border-border bg-white/[0.03] p-3">
          <span className="block text-[9px] font-black uppercase tracking-widest text-white/35 mb-2">
            Exact workout time
          </span>
          <input
            type="time"
            value={
              isValidWorkoutTime(workoutStartTime || "") ? workoutStartTime : ""
            }
            onChange={(event) =>
              setWorkoutStartTimeForDate?.(event.target.value)
            }
      ...
      {plannedExercises.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {plannedExercises.map((ex: any, idx: number) => (
            <div
              key={idx}
              className="rounded-2xl border border-border bg-white/[0.02] p-4 space-y-3"
            >
              <div className="flex justify-between gap-3">
                <div>
                  <div className="text-sm font-black">{ex.name}</div>
                  <div className="label-small text-lime mt-1">
                    {ex.bodyPart}
                  </div>
                </div>
                <button
              ...
              <div className="text-[10px] text-white/40 font-bold uppercase tracking-widest">
                Suggested: {suggestSets()} sets •{" "}
                {suggestedRepsForExercise(ex.name, ex.bodyPart)} reps
              </div>

              <div className="grid grid-cols-3 gap-2">
                <button
                  onClick={() => startPlanExercise(ex)}
                  className="py-2 rounded-xl bg-lime/15 text-lime border border-lime/20 text-[9px] font-black uppercase hover:bg-lime hover:text-dark transition-all"
                >
                  Log Set
                </button>
                <button
      ...
      {planEditorOpen && (
        <div className="rounded-[2rem] border border-border bg-black/20 p-5 space-y-5">
            ...
            <div className="max-h-40 overflow-y-auto space-y-2 custom-scrollbar">
              {getExerciseOptions(planBodyPartInput)
                .filter(
                  (e: string) =>
                    !planExerciseInput ||
                    e.toLowerCase().includes(planExerciseInput.toLowerCase()),
                )
                .slice(0, 10)
                .map((e: string) => (
                  <button
          ...
          {(currentPlanDraft.exercises || []).length > 0 && (
            <div className="space-y-2">
              {(currentPlanDraft.exercises || []).map(
                (ex: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex justify-between items-center rounded-xl border border-border bg-white/[0.02] px-4 py-3"
                  >
                    <div>
...
function StrengthPanel(props: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-2">
      <div>
        <div className="label-small text-muted mb-4 ml-1">
          Target Muscle Group
        </div>
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-3 text-left">
                <div
                  className={`w-1.5 h-1.5 rounded-full mb-1 transition-all ${
                    selectedMuscle === m
                      ? "bg-lime shadow-[0_0_8px_rgba(215,255,0,1)]"
                      : "bg-white/20"
                  }`}
                />
                <div
                  className={`text-[10px] font-black uppercase tracking-widest ${selectedMuscle === m ? "text-lime" : "text-white/60"}`}
                >
                  {m}
                </div>
              </div>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            {exercise && (
              ...
              >
                <div className="text-[10px] font-black text-lime uppercase tracking-widest mb-1">
                  Selected Exercise
                </div>
                ...
                {previousPerformance ? (
                  <>
                    <div className="grid grid-cols-2 gap-2 mt-4">
                      <MiniMetric
                        label="Last Weight"
                        value={`${previousPerformance.lastWeight} kg`}
                      />
                      <MiniMetric
                        label="Last Reps"
                        value={previousPerformance.lastReps}
                      />
                      <MiniMetric
                        label="Best Set"
                        value={previousPerformance.bestSet}
                      />
                      <MiniMetric
                        label="Suggested"
                        value={previousPerformance.suggested}
                      />
                    </div>
                  ...
                  <p className="text-[10px] text-white/30 mt-1">
                    No previous performance found. Start with a controlled first
                    set.
                  </p>
                )}
              </motion.div>
          ...
          {personalExercises.filter((e: any) => e.bodyPart === selectedMuscle)
            .length > 0 && (
              ...
              {personalExercises
                ...
                .map((e: any) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleDeletePersonalExercise(e.name)}
                      className="text-white/20 hover:text-pink transition-colors"
                    >
                      <X size={10} />
                    </button>
                  </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-black uppercase text-lime tracking-[0.2em]">
                      Muscle Focus
                    </span>
                  </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center mb-2 px-1">
        ...
        <span
          className={`text-[10px] font-black uppercase ${Number(rpe) > 8 ? "text-pink" : "text-lime"}`}
        >
              ...
              >
                ...
                {editingSetId === s.id ? (
                  <>
                    ...
                    <td className="p-4 text-xs font-black text-white/70">
                      {getSetVolume({
                        ...s,
                  ...
                  <>
                    <td className="p-4 text-xs font-black text-lime">
                      {s.weight === "0" ? "BW" : `${s.weight} kg`}
                    </td>
                    <td className="p-4 text-xs font-black">{s.reps}</td>
                    <td className="p-4 text-xs font-bold">
                      RPE {s.rpe} • {RPE_LABELS[String(s.rpe)] || ""}
                    </td>
                    <td className="p-4 text-xs font-black text-white/70">
                      {getSetVolume(s)} kg
                    </td>
...
function CardioPanel(props: any) {
  ...
  const estimatedCalories = durationNum
    ? Math.round(
        durationNum * getCardioCaloriesPerMinute(cardioExercise, fatigueStatus),
      )
    : 0;
  const cardioAdvice = getConditioningAdvice(
    readiness,
    fatigueStatus,
    "Cardio",
  );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-2 transition-all">
      <div className="rounded-3xl border border-sky/20 bg-sky/5 p-5">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 text-[10px] font-black uppercase tracking-widest text-white/40">
          <div>
            Readiness: <span className="text-lime">{readiness}%</span>
          </div>
          <div>
            Est. Burn:{" "}
            <span className="text-sky">{estimatedCalories || "—"} kcal</span>
          </div>
        </div>
      </div>

      <div className="space-y-6">
        <div className="label-small text-muted ml-1">
          Primary Cardio Options
        </div>
        ...
        <InputText
          label="Manual Selection / Search"
          value={cardioExercise}
          onChange={(v: string) => setCardioExercise(toExerciseTitle(v))}
      ...
      >
        ...
        {cardioExtraMetric && (
          <div className="space-y-2 animate-in zoom-in-95">
            <div className="label-small text-sky ml-1 font-black">
              {cardioExtraMetric}
            </div>
            <input
              type="text"
      ...
      >
        ...
        {cardioExercise
          ? `Log ${toExerciseTitle(cardioExercise)}`
          : "Log Cardio"}
      </button>
    </div>
  );
...
function SportsYogaPanel(props: any) {
  ...
  const estimatedCalories = durationNum
    ? Math.round(
        durationNum *
          (mode === "Sports"
            ? getCardioCaloriesPerMinute(cardioExercise || mode, fatigueStatus)
            : 4),
      )
    : 0;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-2 transition-all">
      ...
      <div className="space-y-4">
        <div className="label-small text-muted ml-1">
          {mode === "Sports" ? "Sports Disciplines" : "Yoga Styles"}
        </div>
      ...
      <div className="space-y-4">
        <div className="label-small text-muted ml-1">
          {mode === "Sports" ? "Select Activity" : "Select Asana / Sequence"}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-hide">
        {(mode === "Sports" ? [30, 45, 60, 90] : [15, 30, 45, 60]).map(
          (min) => (
            <button
              key={min}
              onClick={() => setCardioDuration(String(min))}
    ...
    <div id="workout-history" className="stat-card">
      ...
      {workoutsArr.length === 0 ? (
        <div className="py-16 text-center">
          <div className="label-small opacity-20 italic">
            ...
            >
              ...
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest opacity-40">
                ...
                <span className="text-lime">
                  {getWorkoutTotalVolume(w.sets || [])} kg volume
                </span>
              </div>
            </div>
          ))}
          ...
          >
            ...
            <div className="space-y-5">
              <InputText
                ...
                onChange={(v: string) =>
                  setCustomExercise({ ...customExercise, name: v })
                }
                placeholder="Enter exercise name..."
              />

              <div className="space-y-2">
                <label className="label-small text-muted ml-1 uppercase opacity-40">
                  Body Part
                </label>
                <select
...
function RestTimer({ time, setTime, isActive, setIsActive, reason }: any) {
  ...
  useEffect(() => {
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification("StayFitInLife", {
        body: "Rest complete. Start your next set.",
      });
    }
  ...
  return (
    <div className="stat-card">
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button
          onClick={reset}
          className="px-4 rounded-xl bg-white/5 border border-border text-white/50 hover:text-white text-[10px] font-black uppercase"
        >
          <RotateCcw size={14} />
          Reset
        </button>
