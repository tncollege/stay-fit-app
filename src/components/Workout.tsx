...
const allWorkouts = useMemo<Workout[]>(() => {
  ...
  const inferExerciseBodyPart = (exerciseName: string, fallback = planBodyPartInput) => {
    const cleanName = toExerciseTitle(exerciseName);
    const matchedBodyPart = BODY_PARTS.find((part) =>
      getExerciseOptions(part).some((option: string) => option.toLowerCase() === cleanName.toLowerCase())
    );
    return matchedBodyPart || fallback || 'Chest';
  };

  const replaceExerciseInPlan = (index: number, replacementName: string) => {
    const name = toExerciseTitle(replacementName);
    if (!name) return;

    const existing = currentPlanDraft.exercises || [];
    const duplicate = existing.some(
      (ex: any, i: number) => i !== index && String(ex.name || '').toLowerCase() === name.toLowerCase()
    );
    if (duplicate) {
      showWorkoutMessage(`${name} is already in this plan.`, 'error');
      return;
    }

    const previousExercise = existing[index] || {};
    const bodyPart = inferExerciseBodyPart(name, previousExercise.bodyPart || planBodyPartInput);
    const updated = existing.map((ex: any, i: number) =>
      i === index
        ? {
            ...ex,
            name,
            bodyPart,
            target: bodyPart,
            suggestedSets: ex.suggestedSets || suggestSets(),
            suggestedReps: suggestedRepsForExercise(name, bodyPart),
          }
        : ex
    );
  ...
  const moveExerciseInPlan = (fromIndex: number, toIndex: number) => {
    const existing = currentPlanDraft.exercises || [];
    if (fromIndex === toIndex || fromIndex < 0 || toIndex < 0 || fromIndex >= existing.length || toIndex >= existing.length) return;
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="stat-card space-y-8">
              ...
              <WeeklyPlanSection
                ...
                replaceExerciseInPlan={replaceExerciseInPlan}
                moveExerciseInPlan={moveExerciseInPlan}
...
function WeeklyPlanSection(props: any) {
  ...
  const allReplacementOptions = useMemo(() => {
    return Array.from(
      new Set(
        BODY_PARTS.flatMap((part) => getExerciseOptions(part)).map((option: string) => toExerciseTitle(option))
      )
    ).sort((a: string, b: string) => a.localeCompare(b));
  }, [getExerciseOptions]);

  const commitExerciseReplacement = (index: number, value: string) => {
    const cleanValue = toExerciseTitle(value);
    if (!cleanValue) return;
    const exactMatch = allReplacementOptions.find((option: string) => option.toLowerCase() === cleanValue.toLowerCase());
      ...
      {planEditorOpen && (
        <div className="rounded-[2rem] border border-border bg-black/20 p-5 space-y-5">
          ...
          {(currentPlanDraft.exercises || []).length > 0 && (
            <div className="space-y-3">
              <div className="rounded-2xl border border-sky/15 bg-sky/5 px-4 py-3 text-[10px] font-bold text-sky/90 leading-relaxed">
                Drag the handle to change sequence, or replace any exercise from the field. Changes stay in draft until you tap Save Weekly Plan.
              </div>
              {(currentPlanDraft.exercises || []).map((ex: any, idx: number) => (
                <div
                  ...
                  onDrop={() => {
                    ...
                    moveExerciseInPlan(draggedExerciseIndex, idx);
                    setDraggedExerciseIndex(null);
                  }}
                  onDragEnd={() => setDraggedExerciseIndex(null)}
                  className={`rounded-2xl border bg-white/[0.02] p-3 transition-all ${
                    draggedExerciseIndex === idx ? 'border-lime/60 opacity-60 scale-[0.99]' : 'border-border'
                  }`}
                >
                  <div className="grid grid-cols-[auto_1fr_auto] gap-3 items-center">
                    <button
                      ...
                      className="cursor-grab active:cursor-grabbing p-3 rounded-xl bg-white/[0.04] border border-border text-white/35 hover:text-lime hover:border-lime/30 transition-all"
                    >
                    ...
                    <div className="space-y-2 min-w-0">
                      <div className="flex items-center justify-between gap-3">
                        <div className="label-small text-lime">#{idx + 1} • {ex.bodyPart}</div>
                        <div className="hidden sm:flex gap-1">
                          <button
                            type="button"
                            onClick={() => moveExerciseInPlan(idx, idx - 1)}
                            disabled={idx === 0}
                            className="px-2 py-1 rounded-lg bg-white/[0.03] border border-border text-[9px] font-black text-white/40 disabled:opacity-20 hover:text-white"
                          >
                            ↑
                          </button>
                          <button
                            type="button"
                            onClick={() => moveExerciseInPlan(idx, idx + 1)}
                            disabled={idx === (currentPlanDraft.exercises || []).length - 1}
                            className="px-2 py-1 rounded-lg bg-white/[0.03] border border-border text-[9px] font-black text-white/40 disabled:opacity-20 hover:text-white"
                          >
                            ↓
                          </button>
                    ...
                    <button
                      ...
                      className="p-3 rounded-xl bg-pink/20 text-pink border border-pink/30 hover:bg-pink hover:text-dark transition-all"
